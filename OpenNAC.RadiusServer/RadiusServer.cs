...
namespace OpenNAC.RadiusServer
{
    public class RadiusServer : IDisposable
    {
        ...
        internal IRadiusPacket GetResponsePacket(IPacketHandler packetHandler, string sharedSecret, byte[] packetBytes, IPEndPoint remoteEndpoint)
        {
            ...
            _logger.LogInformation($"Received {requestPacket.PacketType} from {remoteEndpoint} Id={requestPacket.Identifier}");
            ...
            if (requestPacket.PacketType == RadiusPacketType.StatusServer)
            {
                var responseCode = _serverType == RadiusServerType.Authentication ? RadiusPacketType.AccessAccept : RadiusPacketType.AccountingResponse;
            ...
            var responsePacket = packetHandler.HandlePacket(remoteEndpoint.Address, requestPacket);
            sw.Stop();

            _logger.LogDebug($"{remoteEndpoint} Id={responsePacket.Identifier}, Received {responsePacket.PacketType} from handler in {sw.ElapsedMilliseconds}ms");
        ...
        private void SendResponsePacket(IRadiusPacket responsePacket, IPEndPoint remoteEndpoint)
        {
            if (_server != null)
            {
                ...
                _logger.LogInformation($"{responsePacket.PacketType} sent to {remoteEndpoint} Id={responsePacket.Identifier}");
            }
        }
        ...
        private void DumpPacket(IRadiusPacket packet)
        {
            ...
            foreach (var attribute in packet.Attributes)
            {
                ...
                {
                    var value = attribute.Value.First().ToString();
                    int length = value != null ? value.Length : 0;

                    sb.AppendLine($"{attribute.Key} length : {length}");
                }
                else
                {
